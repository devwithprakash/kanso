CREATE TYPE "public"."field_type" AS ENUM('text', 'textarea', 'email', 'number', 'phone', 'select', 'radio', 'checkbox', 'date', 'rating', 'file');--> statement-breakpoint
CREATE TABLE "users" (
...
ALTER TABLE "field_options" ADD CONSTRAINT "field_options_field_id_form_fields_id_fk" FOREIGN KEY ("field_id") REFERENCES "public"."form_fields"("id") ON DELETE no action ON UPDATE no action;--> statement-breakpoint
